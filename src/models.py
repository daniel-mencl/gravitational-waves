...
class EncoderTransformer(nn.Module):
    ...
    def forward(self, src):
        src_embedded = self.dropout(self.positional_encoding(
            src.reshape(-1, self.max_seq_length, 1)))
...
def generate_mask(tgt):
    ...
    no_peek_mask = (1 - torch.triu(torch.ones(1, seq_length,
                    seq_length, device=tgt.device), diagonal=1)).bool()
    return no_peek_mask
...
class DecoderTransformer(nn.Module):
    ...
    def forward(self, tgt):
        ...
        tgt_embedded = self.dropout(self.positional_encoding(
            tgt.reshape(-1, self.max_seq_length, 1)))
